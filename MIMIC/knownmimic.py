...
# Import dataset after appropriately replacing the file path
mimic = pd.read_csv('input/mimic3/MIMICtable_261219.csv')
mimic.head()

# Rewards calculation for Hypotension problem, using the methodology followed in https://arxiv.org/pdf/2002.03478
...
# Define your columns
state_columns = ['Creatinine','FiO2_1','Arterial_lactate','paO2','paCO2','output_4hourly','GCS','Calcium','Chloride','Glucose','HCO3','Magnesium','Potassium','Sodium','SpO2']
...
for col in state_columns:
    ...
    # Quantize
    mimic[f'quantized_{col}'] = pd.cut(mimic[f'{col}'], bins=10, labels=[i for i in range(10)])
...
for icu_id, group in icu_groups:
    ...
    #hr_values = group['HR'].tolist()
    #sysbp_values = group['SysBP'].tolist()
    #diabp_values = group['DiaBP'].tolist()
    ...
    #qhr_values = group['quantized_HR'].tolist()
    #qsysbp_values = group['quantized_SysBP'].tolist()
    #qdiabp_values = group['quantized_DiaBP'].tolist()
    ...
    #nhr_values = group['normalized_HR'].tolist()
    #nsysbp_values = group['normalized_SysBP'].tolist()
    #ndiabp_values = group['normalized_DiaBP'].tolist()
    ...
    #hr_trajectories.append(hr_values)
    #sysbp_trajectories.append(sysbp_values)
    #diabp_trajectories.append(diabp_values)
    ...
    #qhr_trajectories.append(qhr_values)
    #qsysbp_trajectories.append(qsysbp_values)
    #qdiabp_trajectories.append(qdiabp_values)
    ...
    #nhr_trajectories.append(nhr_values)
    #nsysbp_trajectories.append(nsysbp_values)
    #ndiabp_trajectories.append(ndiabp_values)
...
# This completes the trajectories generation
...
# Evaluating behavior and known concept policies using ANNs 
def predict_actions(index, state, num_neighbors):
...
behavior_neighbors = 200

import random
from operator import itemgetter

# Known concept Importance Sampling
...
# Concept-based Importance Sampling
...
# Known Concept evaluations

Metrics = {}
estimators = [IS, PDIS, CIS1, CPDIS1]
...
for n_eps in [50,100,150,200,250, 300, 350, 400, 450, 500]:
    ...
    Metrics[str(n_eps)] = {}
    Metrics[str(n_eps)][IS.__name__] = {}
    Metrics[str(n_eps)][IS.__name__]['Variance'] = []
    Metrics[str(n_eps)][PDIS.__name__] = {}
    Metrics[str(n_eps)][PDIS.__name__]['Variance'] = []
    Metrics[str(n_eps)][CIS1.__name__] = {}
    Metrics[str(n_eps)][CIS1.__name__]['Variance'] = []
